...
JetMETHLTOfflineSource::bookHistograms(DQMStore::IBooker & iBooker, edm::Run const & run, edm::EventSetup const & c)
{
  ...
  if(!isSetup_){
	...
	if((v->getObjectType() == trigger::TriggerJet) && (v->getTriggerType().compare("SingleJet_Trigger") == 0)){   
	  ...
	  std::string subsubdirName = subdirName + "/CaloJet";
	  iBooker.setCurrentFolder(subsubdirName);
	  ...
	  subsubdirName = subdirName + "/PFJet";
	  iBooker.setCurrentFolder(subsubdirName);
	...
	if(v->getObjectType() == trigger::TriggerMET || (v->getObjectType() == trigger::TriggerTET)){
	  ...
	  std::string subsubdirName = subdirName + "/MET";
	  iBooker.setCurrentFolder(subsubdirName);
  ...
  if ( genTriggerEventFlagDCS_ && genTriggerEventFlagDCS_->on() ) genTriggerEventFlagDCS_->initRun( run, c );
