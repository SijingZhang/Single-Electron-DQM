...
//    book1D(iBooker, "efficiencyZ0_" + suffix, "z0", ";z0;");
    ...
    book1D(iBooker, "massVsmassZ_" + suffix, "zMass", ";mass");
...
//                                         plotCuts_[triggerLevel_ + "DeltaR"]);
  ...
  for (size_t i = 0; i < targetElectrons.size(); i++) {
    ...
    float isoPFValueCorr = chad + std::max(0.0f, nhad + pho - Rho*eA);

    // isTightElectron selection(for both Tag and Probe)
    if(!(( (abs(eta) <= 1.479) && full5x5_sigmaIetaIeta < 0.00998 && abs(dEtaSeedClusterTrackAtVtx) < 0.00308 && abs(dPhiSuperClusterTrackAtVtx) < 0.0816 && HOverE < 0.0414 && EInverseMinusPInverse < 0.0129 && mHits <= 1 && isPassConversionVeto && isoPFValueCorr < 0.0588)
      || ( (abs(eta) >= 1.479) && full5x5_sigmaIetaIeta < 0.0292 && (abs(dEtaSeedClusterTrackAtVtx) < 0.00605) && abs(dPhiSuperClusterTrackAtVtx) < 0.0394 && HOverE < 0.0641 && EInverseMinusPInverse < 0.0129 && mHits <= 1 && isPassConversionVeto && isoPFValueCorr < 0.0571))) continue;

// WP Tight selection criteria for only Tag electrons;
//     if(( (abs(eta) <= 1.479) && full5x5_sigmaIetaIeta < 0.00998 && abs(dEtaSeedClusterTrackAtVtx) < 0.00308 && abs(dPhiSuperClusterTrackAtVtx) < 0.0816 && HOverE < 0.0414 && EInverseMinusPInverse < 0.0129 && mHits <= 1 && isPassConversionVeto && isoPFValueCorr < 0.0588)  || ( (abs(eta) >= 1.479) && full5x5_sigmaIetaIeta < 0.0292 && (abs(dEtaSeedClusterTrackAtVtx) < 0.00605) && abs(dPhiSuperClusterTrackAtVtx) < 0.0394 && HOverE < 0.0641 && EInverseMinusPInverse < 0.0129 && mHits <= 1 && isPassConversionVeto && isoPFValueCorr < 0.0571) ){
...
//    cout<<"etaElectron = "<< eta2 <<endl <<"---------"<<endl;
    ...
    for (size_t j = 0; j < 2; j++) {
      ...
      if ( (abs(eta) >= 1.442 && abs(eta) <= 1.566) || electron.pt() <= 20) continue;
    ...
    for (size_t k = 0; k < targetElectrons.size(); k++) {
      ...
      if (electron.charge() != theProbe.charge() && !pairalreadyconsidered) {
        ...
        if(mass > 60 && mass < 120) {
          ...
          hists_["massVsmassZ_denom"]->Fill(mass);
          ...
          if(matches[k] < targetElectrons.size()) {
            hists_["massVsmassZ_numer"]->Fill(mass);
